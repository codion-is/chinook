...
if aws lambda get-function --function-name chinook-lambda &> /dev/null; then
    ...
    aws lambda create-function \
        ...
        --timeout 30 \
        --memory-size 512 \
        --environment 'Variables={"JAVA_TOOL_OPTIONS":"-Dcodion.db.url=jdbc:h2:mem:h2db -Dcodion.db.initScripts=classpath:create_schema.sql -Dcodion.db.countQueries=true -Dcodion.server.connectionPoolUsers=scott:tiger -Dcodion.server.objectInputFilterFactoryClassName=is.codion.common.rmi.server.SerializationFilterFactory -Dcodion.server.serialization.filter.patternFile=classpath:serialization-filter-patterns.txt -Dcodion.server.idleConnectionTimeout=600000"}' > /dev/null
